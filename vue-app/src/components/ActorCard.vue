<template>
  ...
  >
    ...
    <button
        type="button"
        :class="[
        'absolute right-2 top-2 z-10 flex items-center gap-1 rounded-full px-2 py-1 text-[0.72rem] font-medium shadow-sm ring-1 transition-all duration-200',
        favorited
          ? 'opacity-100 translate-y-0 bg-emerald-50 text-emerald-700 ring-emerald-200 cursor-default'
          : 'opacity-0 translate-y-[-2px] group-hover:opacity-100 group-hover:translate-y-0 bg-white/90 text-rose-600 ring-rose-200/70 hover:bg-white cursor-pointer'
      ]"
        :disabled="favorited || saving"
        aria-label="Favoritar ator"
        :title="favorited ? 'Favorito' : (saving ? 'Salvando...' : 'Clique para favoritar')"
    ...
    >
      ...
      <span class="leading-none">
        {{ favorited ? 'Favorito' : (saving ? 'Salvando...' : 'Clique para favoritar') }}
      </span>
    </button>
    ...
    <div class="text-center">
      ...
      <p v-if="actor.sexo" class="mt-1 text-[0.9rem] text-[#4b5563]">Sexo: {{ actor.sexo }}</p>
      <p v-if="actor.idade" class="mt-1 text-[0.9rem] text-[#4b5563]">Idade: {{ actor.idade }}</p>
    </div>
  </article>
</template>
...
function onFavClick() {
  if (props.favorited || props.saving) return
